...
flowchart LR
  subgraph Operators["CrownLabs Deployments<br><small>(ArgoCD applications)</small>"]
    PROD["crownlabs-production<br/>Production"]
    PRE["crownlabs-pre-production<br/>Pre-production"]
    PR["crownlabs-staging-123<br/>Staging (pr-associated)"]
  end

  Argo["Helm / ArgoCD"] --> PROD
  Argo --> PRE
  Argo --> PR

  subgraph Controllers["Controllers that handle <code>operator-selector=production</code>"]
    WC["Workspace Controller"]
    TC["Tenant Controller"]
    IC["Instance Controller"]
    BASTION["SSH Bastion"]
    WEBSSH["WebSSH"]
    CLFE["CrownLabs Frontend"]
    QLK["QLKube"]
  end
  ...
  WCR["Workspace CR<br/>workspace: abc"]

  subgraph WorkspaceNS["Workspace NS: workspace-abc"]
    subgraph TPL["Template foo"]
      ENV1["Environment 1<br><small>(container based)</small>"]
      ENV2["Environment 2<br><small>(vm based)</small>"]
    end
    TPL2["Another Template<br><small>(does nothing if not referenced in an instance)</small>"]
  end

  WC --reconciles--> WCR
  WCR --> WorkspaceNS

  TCR["Tenant CR<br/>tenant: xyz.efg<br/><code>operator-selector=production</code>"]

  subgraph TenantNS["Tenant NS: tenant-xyz-efg (has label for <code>operator-selector=production</code>)"]
    subgraph ENV2Cont["Instanced environment 2"]
      ROUTE2["Ingress / HTTPRoute"]
      SVC2["Service"]
      VM["VirtualMachine"]
    end
      VLPOD["Kubevirt VirtLauncher Pod"]
    INST["Instance-bar"]
    subgraph ENV1Cont["Instanced environment 1"]
  ...
  TC -- reconciles --> TCR
  BASTION -. watches .-> TCR
  TCR -- manages --> TenantNS

  INST -. references .-> TPL
  TPL -. defines .-> INST
  IC -- reconciles --> INST
  INST -- becomes --> ENV1Cont
  INST -- becomes --> ENV2Cont
  ENV1 -. configures .-> DEPLOY
  ENV2 -. configures .-> VM
  VM -- becomes --> VLPOD
