        ...
        return (
            <li key={x} className={classes.listSection}>
                <ul className={classes.ul}>
                    ...
                    >
                        <ListItemText style={{backgroundColor: color}} inset
                                      primary={x.charAt(0).toUpperCase() + x.slice(1).replace(/-/g, " ")}/>
                        {selectedIndex == index ?
                            <Tooltip title="Stop VM">
                                <IconButton style={{color: "red"}} button="true" onClick={() => {
                                    props.stop();
                                    setSelectedIndex(-1)
                                }}>
                        ...
                        {(selectedIndex == index && status === 1) ?
                            <Tooltip title="Connect VM">
                                <IconButton style={{color: "black"}} button="true" onClick={() => {
                                    props.connect();
                                    setSelectedIndex(-1);
                                }}>
