...
- **`ray-worker/`**: Contains a Dockerfile to build a custom Ray worker image with pre-installed dependencies. Using this custom image reduces startup time when leveraging `runtime_env` in your Python scripts. The image contains the following pre-installed packages:
  - `torch==2.4.1`
  - `torchvision==0.19.1`
  - `torchaudio==2.4.1`