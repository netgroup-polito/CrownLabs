...
function decorateSchema(baseSchema) {
  ...
  const allType = new GraphQLObjectType({
    ...
    fields: {
      services: createType({
        name: 'services',
        allNamespaceQueryName: 'listCoreV1ServiceForAllNamespaces',
        namespacedQueryName: 'ioK8sApiCoreV1ServiceList',
        baseSchema,
      }),
      deployments: createType({
        name: 'deployments',
        allNamespaceQueryName: 'ioK8sApiAppsV1DeploymentList',
        namespacedQueryName: 'listAppsV1NamespacedDeployment',
        baseSchema,
      }),
      pods: createType({
        name: 'pods',
        allNamespaceQueryName: 'listCoreV1PodForAllNamespaces',
        namespacedQueryName: 'ioK8sApiCoreV1PodList',
        baseSchema,
      }),
      daemonSets: createType({
        name: 'daemonSets',
        allNamespaceQueryName: 'ioK8sApiAppsV1DaemonSetList',
        namespacedQueryName: 'listAppsV1NamespacedDaemonSet',
        baseSchema,
      }),
      replicaSets: createType({
        name: 'replicaSets',
        allNamespaceQueryName: 'listAppsV1ReplicaSetForAllNamespaces',
        namespacedQueryName: 'ioK8sApiAppsV1ReplicaSetList',
        baseSchema,
      }),
      statefulSets: createType({
        name: 'statefulSets',
        allNamespaceQueryName: 'listAppsV1StatefulSetForAllNamespaces',
        namespacedQueryName: 'ioK8sApiAppsV1StatefulSetList',
        baseSchema,
      }),
      jobs: createType({
        name: 'jobs',
        allNamespaceQueryName: 'ioK8sApiBatchV1JobList',
        namespacedQueryName: 'listBatchV1NamespacedJob',
        baseSchema,
      }),
      cronJobs: createType({
        name: 'cronJobs',
        allNamespaceQueryName: 'ioK8sApiBatchV1beta1CronJobList',
        namespacedQueryName: 'listBatchV1beta1NamespacedCronJob',
        baseSchema,
      }),
      namespaces: createNamespaceType({
        name: 'namespaces',
        allNamespaceQueryName: 'ioK8sApiCoreV1NamespaceList',
        baseSchema,
      }),
      configmaps: createType({
        name: 'configmaps',
        allNamespaceQueryName: 'ioK8sApiCoreV1ConfigMapList',
        namespacedQueryName: 'listCoreV1NamespacedConfigMap',
        baseSchema,
      }),
    },
  });
...
function createType({
  name,
  allNamespaceQueryName,
  namespacedQueryName,
  baseSchema,
}) {
  const allNamespaceQueryType = baseSchema.getQueryType().getFields()[
    allNamespaceQueryName
  ];
  const namespacedQueryType = baseSchema.getQueryType().getFields()[
    namespacedQueryName
  ];
  return {
...
function createNamespaceType({ allNamespaceQueryName, baseSchema }) {
  ...
  return {
    ...
    async resolve(parent, args, context, info) {
      const namespacesProm = allNamespaceQueryType.resolve(
        parent,
        parent,
        context,
        info
      ...
      if (parent.namespace) {
        // if 'namespace' argument provided to `all` query, then filter items in namespace list
        const namespaces = await namespacesProm;
        namespaces.items = namespaces.items.filter(
          ns => ns.metadata.name === parent.namespace
        );
        return namespaces;
      }
