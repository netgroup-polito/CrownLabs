type envVarNames =
  | 'BASE_URL'
  | 'VITE_APP_CROWNLABS_GRAPHQL_URL'
  | 'VITE_APP_CROWNLABS_OIDC_CLIENT_ID'
  | 'VITE_APP_CROWNLABS_OIDC_AUTHORITY'
  | 'VITE_APP_MYDRIVE_TEMPLATE_NAME'
  | 'VITE_APP_MYDRIVE_WORKSPACE_NAME'
  | 'VITE_APP_CROWNLABS_GROUPS_CLAIM_PREFIX'
  | 'VITE_APP_CROWNLABS_GROUPS_ADMIN_CLAIM'
  | 'VITE_APP_CROWNLABS_GRAFANA_DASHBOARD_URL'
  | 'VITE_APP_CROWNLABS_BASTION_HOST'
  | 'VITE_APP_CROWNLABS_IMAGE_CREATION_WARNING'
  | 'CROWNLABS_IMAGE_CREATION_HELP'
  | 'VITE_APP_CROWNLABS_PUBLIC_REGISTRY_NAME_DESTINATION'
  | 'VITE_APP_CROWNLABS_IMAGELIST_PUBLIC_SNAPSHOTS'
  | 'VITE_APP_CROWNLABS_IMAGELIST_STANDALONE'
  | 'VITE_APP_CROWNLABS_IMAGELIST_CONTAINERDISKS'
  | 'VITE_APP_CUSTOM_RESOURCES';

type envVarObj = { [key in envVarNames]?: string };

export const getEnvVar = (envVarName: envVarNames): string => {
  const envVar: string =
    import.meta.env[envVarName] ?? (window as envVarObj)[envVarName];
  if (envVar === undefined) {
    throw new Error(`ERROR: ENV VAR ${envVarName} NOT DEFINED`);
  }
  return envVar;
};

export const VITE_APP_CROWNLABS_OIDC_CLIENT_ID = getEnvVar(
  'VITE_APP_CROWNLABS_OIDC_CLIENT_ID',
);
export const VITE_APP_CROWNLABS_GRAPHQL_URL = getEnvVar(
  'VITE_APP_CROWNLABS_GRAPHQL_URL',
);
export const VITE_APP_CROWNLABS_OIDC_AUTHORITY = getEnvVar(
  'VITE_APP_CROWNLABS_OIDC_AUTHORITY',
);
export const VITE_APP_CROWNLABS_GROUPS_CLAIM_PREFIX = getEnvVar(
  'VITE_APP_CROWNLABS_GROUPS_CLAIM_PREFIX',
);
export const VITE_APP_CROWNLABS_GROUPS_ADMIN_CLAIM = getEnvVar(
  'VITE_APP_CROWNLABS_GROUPS_ADMIN_CLAIM',
);
export const VITE_APP_CROWNLABS_GRAFANA_DASHBOARD_URL = getEnvVar(
  'VITE_APP_CROWNLABS_GRAFANA_DASHBOARD_URL',
);
export const VITE_APP_CROWNLABS_BASTION_HOST = getEnvVar(
  'VITE_APP_CROWNLABS_BASTION_HOST',
);
export const VITE_APP_CROWNLABS_IMAGE_CREATION_WARNING = getEnvVar(
  'VITE_APP_CROWNLABS_IMAGE_CREATION_WARNING',
);
export const CROWNLABS_IMAGE_CREATION_HELP =
  import.meta.env.CROWNLABS_IMAGE_CREATION_HELP ??
  (window as envVarObj).CROWNLABS_IMAGE_CREATION_HELP ??
  'https://www.youtube.com/watch?v=i7fqga7xQv0';
export const VITE_APP_CROWNLABS_PUBLIC_REGISTRY_NAME_DESTINATION = getEnvVar(
  'VITE_APP_CROWNLABS_PUBLIC_REGISTRY_NAME_DESTINATION',
);
export const VITE_APP_CROWNLABS_IMAGELIST_PUBLIC_SNAPSHOTS = getEnvVar(
  'VITE_APP_CROWNLABS_IMAGELIST_PUBLIC_SNAPSHOTS',
);
export const BASE_URL = getEnvVar('BASE_URL');

export const VITE_APP_MYDRIVE_TEMPLATE_NAME = getEnvVar(
  'VITE_APP_MYDRIVE_TEMPLATE_NAME',
);
export const VITE_APP_MYDRIVE_WORKSPACE_NAME = getEnvVar(
  'VITE_APP_MYDRIVE_WORKSPACE_NAME',
);

export const VITE_APP_CROWNLABS_IMAGELIST_STANDALONE = getEnvVar(
  'VITE_APP_CROWNLABS_IMAGELIST_STANDALONE',
);
export const VITE_APP_CROWNLABS_IMAGELIST_CONTAINERDISKS = getEnvVar(
  'VITE_APP_CROWNLABS_IMAGELIST_CONTAINERDISKS',
);

export const VITE_APP_CUSTOM_RESOURCES = getEnvVar('VITE_APP_CUSTOM_RESOURCES');
