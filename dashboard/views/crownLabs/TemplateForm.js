...
function TemplateForm(props){
  ...
  const addTemplate = () => {
    ...
    const labNums = props.templates.filter(item =>
      item.metadata.namespace === namespace && item.metadata.name.includes(selectedCourse + '-lab')).length;
    let labNumber = labNums + 1;

    if(!selectedCourse || !selectedImage || !selectedVersion ||
      description.current === '' || VMType.current === ''
    ){
    ...
    const item = {
      ...
      spec: {
        ...
        environmentList: [{
          ...
          image: registry.current + '/' + selectedImage + ':' + selectedVersion,
          name: selectedCourse + '-lab' + labNumber,
          persistent: false,
          resources: {
            cpu: cpu,
  ...
  useEffect(() => {
    ...
    setSelectedVersion([]);
    if(selectedImage){
      images.find(item => item.value === selectedImage).versions.forEach( v =>
        setVersions(prev => [...prev, {value: v}])
      )

      if(images.find(item => item.value === selectedImage).versions.length === 1)
        setSelectedVersion(images.find(item => item.value === selectedImage).versions[0]);
    }
  }, [selectedImage])

  useEffect(() => {
    window.api.getGenericResource('/apis/crownlabs.polito.it/v1alpha1/imagelists/crownlabs-virtual-machine-images')
      .then(res =>{
        registry.current = res.spec.registryName;
        res.spec.images.forEach(image => {
          setImages(prev => [...prev, {value: image.name, versions: image.versions}]);
        })
      }).catch(() => {})
    ...
    if(props.templates.length !== 0){
      ...
      props.templates.forEach(template => {
        if(!workspaces.includes(template.spec['workspace.crownlabs.polito.it/WorkspaceRef'].name))
      ...
      if(workspaces.length === 1)
        setSelectedCourse(workspaces[0]);
  ...
  return (
    ...
    >
      <Form {...formItemLayout}>
        ...
        <Form.Item label={'VM'}>
          <Input.Group compact>
            <Input placeholder={'Description'} style={{ width: '70%' }}
        ...
        <Form.Item label={'CPU (Cores)'}>
          <Slider min={1} max={4} defaultValue={cpu}
                  onChange={value => setCpu(value)} dots
          />
        </Form.Item>
        <Form.Item label={'Reserved CPU (%)'}>
          <Slider min={0} max={100} defaultValue={cpuPerc}
                  onChange={value => setCpuPerc(value)}
          />
        </Form.Item>
        <Form.Item label={'Memory (GB)'}>
          <Slider min={0.5} max={8} defaultValue={memory} step={0.5}
