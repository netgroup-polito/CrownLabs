...
// processPacket processes a packet and extracts SSH connection information.
func processPacket(packet gopacket.Packet, connLock *sync.RWMutex, eventQueue chan ConnectionEvent) {
	// Get IP layer
...
// handleEvent processes an SSH connection event and updates metrics.
	...
	go func() {
		...
		for {
			...
			default:
				...
				processPacket(packet, &connLock, eventQueue)
			}
		}
	}()
