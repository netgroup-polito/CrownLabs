...
// TerminalColors is a list of terminal colors to choose from.
...
func (webCtx *ServerContext) wsHandler(w http.ResponseWriter, r *http.Request) {
	...
	defer func() {
		...
		if localCtx.errorMsg != "" {
			...
			if err := ws.WriteMessage(websocket.TextMessage, msgJSON); err != nil {
				localCtx.logger().Error(err, "WebSocket error on closing connection")
			}
		}
	...
	// Start the bash shell with a custom prompt color
	idx, _ := rand.Int(rand.Reader, big.NewInt(int64(len(TerminalColors))))
	ps1 := TerminalColors[idx.Int64()] + `\u@\h:\w\$ ` + `\[\e[0m\]`
	cmd := "env PS1=$'" + ps1 + "' PROMPT_COMMAND= bash --noprofile --norc -i"
	if err := session.Start(cmd); err != nil {
		localCtx.logger().Error(err, "Failed to start bash")
		localCtx.errorMsg = "Internal server error"
		return
	}
...
func (webCtx *ServerContext) timeoutHandler(localCtx *LocalContext) {
	...
	for {
		...
		case <-ticker.C:
			// Check for timeout
			if time.Since(localCtx.lastUsed.Load().(time.Time)) > webCtx.TimeoutDuration {
