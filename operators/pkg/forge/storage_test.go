...
var _ = Describe("External Volume Mounts and Provisioning Job forging", func() {
	...
	Describe("The forge.MyDriveMountInfo and .GetMyDrivePVCMirrorName functions", func() {
		When("Tenant name is short (<= 55 chars)", func() {
		...
		When("Tenant name is long (> 55 chars)", func() {
			tenantNameLong := strings.Repeat("a", 100)
			expectedTrimAt := 55 - len("-drive")

			It("Should return the correctly trimmed VolumeMount object if names are long", func() {
				expected := corev1.VolumeMount{
					Name:      fmt.Sprintf("%s-drive-mirror", strings.Repeat("a", expectedTrimAt)),
					MountPath: forge.MyDriveVolumeMountPath,
					ReadOnly:  false,
				}
				Expect(forge.MyDriveMountInfo(tenantNameLong)).To(Equal(expected))
			})
		})
	})

	Describe("The forge.ShVolMountInfo and .GetShVolPVCMirrorName functions", func() {
		When("Instance and SharedVolume names are short (<= 27 chars)", func() {
		...
		When("Instance and SharedVolume names are long (> 27 chars)", func() {
			instanceNameLong := strings.Repeat("b", 50)
			shVolNameLong := strings.Repeat("a", 50)
			...
			It("Should return the correctly trimmed VolumeMount object if names are long", func() {
				expected := corev1.VolumeMount{
					Name:      fmt.Sprintf("%s-%s-mirror", strings.Repeat("a", 27), strings.Repeat("b", 27)),
					MountPath: mountInfo.MountPath,
					ReadOnly:  mountInfo.ReadOnly,
				}
