...
// ShVolMountInfo forges the VolumeMount for the mirror of the passed SharedVolumeMountInfo.
	...
	for _, mount := range env.SharedVolumeMounts {
		...
		if err := c.Get(ctx, NamespacedNameFromGenericRef(mount.SharedVolumeRef), &shvol); err != nil {
			return nil, fmt.Sprintf(
				...
				mount.SharedVolumeRef.Namespace,
			), err
		}
...
func MyDrivePVCMirrorName(tenantName string) string {
	// The maximum name length for a Kubernetes resource is 253 characters.
	// But, this name is used also as Volume Name, which maximum length is 63 characters.
	// So, for each part the maximum length is (63 - len("-mirror")) = 55.
	// Source: https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#dns-subdomain-names
	// Source: https://kubernetes.io/docs/reference/kubernetes-api/config-and-storage-resources/volume/#Volume
	return fmt.Sprintf("%s-mirror", LastCharsOf(MyDrivePVCName(tenantName), 55))
}
...
func ShVolPVCMirrorName(shvolName, instanceName string) string {
	// The maximum name length for a Kubernetes resource is 253 characters.
	// But, this name is used also as Volume Name, which maximum length is 63 characters.
	// So, for each part the maximum length is floor( (63 - len("--mirror"))/2 ) = 27.
	// Source: https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#dns-subdomain-names
	// Source: https://kubernetes.io/docs/reference/kubernetes-api/config-and-storage-resources/volume/#Volume
