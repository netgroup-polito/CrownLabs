...
func StandaloneContainer(instance *clv1alpha2.Instance, environment *clv1alpha2.Environment, volumeMountPath string, mountInfos []corev1.VolumeMount) corev1.Container {
	...
	AddEnvVariableToContainer(&standaloneContainer, "CROWNLABS_BASE_PATH", ExposeGUICleanPath(instance, environment))
	...
	if environment.RewriteURL {
		...
		SetContainerReadinessHTTPProbe(&standaloneContainer, GUIPortName, ExposeGUIPath(instance, environment))
	}
