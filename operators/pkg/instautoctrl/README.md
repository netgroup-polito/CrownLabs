...
The **Instance Automation Controller** (`instautoctrl` package) implements the backend logic to manage the lifecycle of instances declared as **Inactive**. 
...
This decision is influenced by a special label that can be added to the associated **Namespace** resource, called `InstanceInactivityIgnoreNamespace`.
If the Namespace carries this label with a `true` value, the controller will ignore the instance, allowing it to remain inactive for an extended period without being stopped or deleted.
...
Note: a single query on Prometheus cannot return more than **11000 data points**. In order to be covered for the worst case scenario of 1-month inactivity period, the query resolution (query step) has been set at 5-minutes.
...
* **InstanceInactivityIgnoreNamespace**: `Namespace` label used to ignore the inactivity termination for all the Instances of the entire `Namespace`. Default value (if omitted) is `false`.
...
* **CustomNumberOfAlertsAnnotation**: Instance annotation that stores the override the default `InstanceMaxNumberOfAlerts` in the **InstanceInactiveTerminationReconciler** for a specific template.
...
### Detailed Behavior
The controller begins by retrieving all the active Instances and it fetches the related **Template** resource. Based on the `DeleteAfter` field of the Template, the maximum lifespan of that Instance is determined. 
...
Using the `DeleteAfter` value, the controller calculates the remaining lifespan of the Instance.
Once this lifespan expires, the Instance will be deleted.
A new `ExpiringWarningNotificationAnnotation` annotation is added to the Instance to let the controller know that the first warning notification has been sent.
Before the instance expiration, specifically, `notificationInterval` time  before, the controller sends a warning email to the tenant informing them that their Instance will be deleted soon. After the same time interval, the controller checks again the remaining time, consequently deleting the Instance and sending a second email to the tenant confirming that the Instance has been deleted.
