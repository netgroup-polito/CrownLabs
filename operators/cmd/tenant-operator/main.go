...
func main() {
	...
	flag.DurationVar(&requeueTimeMinimum, "nc-tenant-operator-rq-time-min", 4*3600*1000000000, "Minimum nanoseconds before requeue of controller.") // 30000000000 for testing
	flag.DurationVar(&requeueTimeMaximum, "nc-tenant-operator-rq-time-max", 8*3600*1000000000, "Maximum nanoseconds before requeue of controller.") // 35000000000 for testing
	flag.DurationVar(&tenantWorkspaceKeepAlive, "nc-tenant-operator-wspc-keep-alive", 8000000000000, "Nanoseconds after last login of tenant during which the tenant workspace should be kept alive: after this period, the controller will attempt to delete the tenant personal workspace.") // 80000000000 for testing
